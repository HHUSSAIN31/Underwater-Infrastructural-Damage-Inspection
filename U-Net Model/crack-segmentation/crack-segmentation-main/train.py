...
def train(opt, model, device):
    ...
    # Dataset
    train_data = RoadCrack(root=f"{opt.data}", image_size=opt.image_size, mask_suffix="")
    test_data = RoadCrack(root=f"{opt.data}", image_size=opt.image_size, mask_suffix="")

    # DataLoader
    train_loader = DataLoader(train_data, batch_size=opt.batch_size,  shuffle=True, drop_last=True)
    test_loader = DataLoader(test_data, batch_size=opt.batch_size, drop_last=True)
...
def parse_opt():
    ...
    parser.add_argument("--weights", type=str, default="weights/focal.pt", help="Pretrained model, default: None")
